...
const App = () => {
  ...
  return (
    <div className={classes.root}>
      <Hero parallax ratio={0.2} mobile={false} />
      ...
      <Main className={classes.container}>
        ...
        <section className={classes.section2}>
          <Container>
            <Grid container columns={{ xs: 1, md: 2 }} spacing={8}>
              {peopleData.map((item, index) => (
                <Grid
                  className={classes.personZoom}
                  item
                  xs={1}
                  md={1}
                  key={`person_${index}`}
                >
                  <Zoom
                    delay={!isMobile ? (index % 2 === 0 ? 0 : 150) : 0}
                    triggerOnce={true}
                  >
